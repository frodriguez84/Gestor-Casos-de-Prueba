...
window.deleteTestCase = function (id) {
    if (confirm('¿Estás seguro de que deseas eliminar este caso de prueba?')) {
        // Encontrar el número de escenario del caso eliminado
        const deletedCase = testCases.find(tc => tc.id === id);
        if (!deletedCase) return;

        const deletedScenarioNumber = parseInt(deletedCase.scenarioNumber, 10);

        // Eliminar el caso de prueba
        testCases = testCases.filter(tc => tc.id !== id);

        // Ajustar la numeración de los escenarios por debajo del eliminado
        testCases.forEach(tc => {
            if (parseInt(tc.scenarioNumber, 10) > deletedScenarioNumber) {
                tc.scenarioNumber = (parseInt(tc.scenarioNumber, 10) - 1).toString();
            }
        });
...
window.viewEvidence = function (id) {
    ...
    if (!testCase || !testCase.evidence || testCase.evidence.length === 0) {
        alert('Este caso no tiene evidencias adjuntas');
        return;
    }
...
window.renderTestCases = function () {
    ...
    // Elimina todas las columnas de variables previas (entre Descripción y Resultado Esperado)
    while (theadRow.children[3] && theadRow.children[3].id === "varsThPlaceholder") {
        theadRow.removeChild(theadRow.children[3]);
    }

    // Elimina cualquier th de variables anterior (entre Descripción y Resultado Esperado)
    while (theadRow.children[3] && theadRow.children[3].textContent !== "Resultado Esperado") {
        theadRow.removeChild(theadRow.children[3]);
    }
    ...
    inputVariableNames.forEach(varName => {
        ...
        th.style.minWidth = '150px'; // Ancho fijo
        th.style.maxWidth = '150px'; // Ancho fijo
        th.classList.add('variable-column'); // Agregar clase específica
    ...
    tbody.innerHTML = filteredCases.map(testCase => {
        ...
        const stepsCount = testCase.steps ? testCase.steps.length : 0;
    ...
    reinitializeDragScroll(); // Reinicializar drag scroll
}
